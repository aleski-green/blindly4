...
let treeCommands = CommandGroup(title: "Accessibility tree", commands: [
    Command("tree", "[--pid PID] [--depth N] [--max-nodes N]") { invocation, context in
    ...
    Command("show", "[--pid PID] [--depth N]") { invocation, context in
        let depth = try invocation.integer("depth", default: 4, minimum: 0)
    ...
    Command("find", "[--title TEXT] [--role ROLE] [--value TEXT] [--description TEXT] [--pid PID] [--depth N] [--limit N]") { invocation, context in
